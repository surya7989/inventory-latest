...
const Dashboard: React.FC<DashboardProps> = ({ onNavigateBilling, onVisitStore, stats, products, customers, vendors, transactions = [], purchases = [], expenses = [], user }) => {
    ...
    // Only count transactions linked to existing customers OR Walk-ins
    const validTransactions = safeTransactions.filter(t => t.customerId === 'WALK-IN' || activeCustomerIds.has(t.customerId));
    ...
    const todayExpensesAmount = expenses
        .filter((e: any) => e.date.split('T')[0] === todayStr)
        .reduce((sum: number, e: any) => sum + (Number(e.amount) || 0), 0);

    const todayNetProfit = todaySales - todayStockCost - todayGst - todayExpensesAmount;
                ...
                <div className="lg:col-span-1 bg-[#2563EB] text-white p-8 rounded-3xl shadow-xl flex flex-col justify-between relative overflow-hidden h-full">
                    <div className="relative z-10 space-y-6">
                        ...
                        <div className="p-6 bg-white/10 rounded-2xl border border-white/10">
                            ...
                            <p className="text-4xl font-black tracking-tight">₹{todaySales.toLocaleString('en-IN')}</p>
                        </div>
                        ...
                        <div className="p-6 bg-white/10 rounded-2xl border border-white/10">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center bg-white/5 p-2 rounded-lg">
                                    ...
                                    <p className="text-sm font-black text-white">₹{todaySales.toLocaleString()}</p>
                                </div>

                                <div className="space-y-2.5 px-2">
                                    <div className="flex justify-between items-center text-[10px]">
                                        <p className="text-blue-200/60 uppercase font-black">(-) Cost of Goods</p>
                                        <p className="font-bold text-white/80">₹{todayStockCost.toLocaleString()}</p>
                                    </div>

                                    <div className="flex justify-between items-center text-[10px]">
                                        <p className="text-blue-200/60 uppercase font-black">(-) Tax Liability</p>
                                        <p className="font-bold text-white/80">₹{todayGst.toLocaleString()}</p>
                                    </div>

                                    {todayExpensesAmount > 0 && (
                                        <div className="flex justify-between items-center text-[10px]">
                                            <p className="text-blue-200/60 uppercase font-black">(-) Operational Exp</p>
                                            <p className="font-bold text-white/80">₹{todayExpensesAmount.toLocaleString()}</p>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex justify-between items-center pt-4 mt-2 border-t border-white/10 px-1">
                                    ...
                                    <p className="text-3xl font-black text-emerald-400">₹{todayNetProfit.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
