...
import { TrendingUp, IndianRupee, Users, ShieldCheck, ArrowUp, ArrowDown, FileText, Download, Calendar, Filter, BarChart3, PieChart, DollarSign, Package, ShoppingCart, ArrowRight } from 'lucide-react';
...
const Analytics: React.FC<AnalyticsProps> = ({ products, customers, vendors, transactions, user }) => {
    ...
    const [expenses] = useLocalStorage<any[]>('nx_expenses', []);

    // Helper to check if a date falls within the selected range - Enhanced for local matching
    const isWithinRange = (dateStr: string) => {
        if (!dateStr) return false;

        // Normalize dateStr to YYYY-MM-DD if it's an ISO string (from expenses) 
        // OR use as is for local strings (from sales)
    ...
    const filteredTransactions = useMemo(() => transactions.filter(t => isWithinRange(t.date)), [transactions, dateRange]);
    const filteredExpenses = useMemo(() => expenses.filter(e => isWithinRange(e.date)), [expenses, dateRange]);
    ...
    const metrics = useMemo(() => {
        ...
        const totalRevenue = validTransactions.reduce((sum, t) => sum + Number(t.total), 0);
        const totalGst = validTransactions.reduce((sum, t) => sum + (Number(t.gstAmount) || 0), 0);
        ...
        const netProfit = totalRevenue - totalGst - totalCogs - totalExpenseAmount;
    ...
    }, [filteredTransactions, filteredExpenses, products, customers]);
    ...
    const handleExport = () => {
        ...
        if (selectedReport === 'sales') {
            ...
            const rows = [
                ...
                ["Expense", "Operating Expenses", metrics.expenses],
                ["Expense", "Total Costs", metrics.cogs + metrics.expenses],
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="p-5 bg-slate-50 rounded-2xl flex justify-between items-center transition-all hover:bg-slate-100/50">
                                            ...
                                            <span className="text-sm font-black text-slate-900">₹{metrics.expenses.toLocaleString()}</span>
                                        </div>
                                        <div className="p-5 bg-rose-50 rounded-2xl flex justify-between items-center border border-rose-100">
                                            ...
                                            <span className="text-lg font-black text-rose-700">₹{(metrics.cogs + metrics.expenses).toLocaleString()}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {customers.sort((a, b) => (b.totalPaid || 0) - (a.totalPaid || 0)).slice(0, 5).map((c, i) => (
                                            <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 font-black text-slate-900 text-sm">₹{c.totalPaid?.toLocaleString()}</td>
                                                <td className="px-6 py-4 text-xs font-black text-rose-500">₹{c.pending?.toLocaleString()}</td>
