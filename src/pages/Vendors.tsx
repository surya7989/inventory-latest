...
const Vendors: React.FC<VendorsProps> = ({ vendors, purchases, onUpdate, onDelete, user }) => {
    ...
    const handleEdit = (vendor: Vendor) => {
        setFormData({
            ...
            email: vendor.email,
            image: vendor.image || '',
            totalBill: (vendor.totalPaid + vendor.pendingAmount).toString(),
            totalPaid: vendor.totalPaid.toString(),
            pendingAmount: vendor.pendingAmount.toString(),
        });
        ...
        return (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                    {[
                        { label: 'Total Paid', value: `₹${selectedVendor.totalPaid.toLocaleString()}`, icon: CreditCard, color: 'text-green-600', bg: 'bg-green-50' },
                        { label: 'Pending Amount', value: `₹${selectedVendor.pendingAmount.toLocaleString()}`, icon: Clock, color: 'text-red-500', bg: 'bg-red-50' },
                                        ...
                                        {purchases.filter(p => p.vendorId === selectedVendorId).length === 0 ? (
                                            ...
                                            purchases.filter(p => p.vendorId === selectedVendorId).map((t, i) => (
                                                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                                                    ...
                                                    <td className="px-8 py-6 text-sm font-black text-slate-900">₹{t.amount.toLocaleString()}</td>
                            ...
                            <div className="pt-8 border-t border-slate-50">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-sm font-black text-green-600">₹{selectedVendor.totalPaid.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-bold text-slate-500">Outstanding Balance</span>
                                        <span className="text-sm font-black text-red-500">₹{selectedVendor.pendingAmount.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filtered.map(v => (
                            <tr key={v.id} className={`hover:bg-slate-50/50 transition-colors ${selectedRows.includes(v.id) ? 'bg-purple-50/30' : ''}`}>
                                ...
                                <td className="px-4 lg:px-6 py-4 font-black text-sm text-blue-600">₹{(v.totalPaid + v.pendingAmount).toLocaleString()}</td>
