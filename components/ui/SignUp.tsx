...
// Added explicit size property to GlassButtonProps to fix inference issues with VariantProps
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", onSuccess, onGoToLogin }: AuthComponentProps) => {
    ...
    <AnimatePresence>
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
  ...
  return (
    <div className="bg-background min-h-screen w-full flex flex-col relative overflow-hidden">
        ...
        <div className="flex w-full flex-1 items-center justify-center bg-card relative">
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-full max-w-[320px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.1} className="w-full text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground">Get started with Us</p></BlurFade>
                        <BlurFade delay={0.2}><p className="text-sm font-medium text-muted-foreground">Continue with</p></BlurFade>
                        ...
                        <BlurFade delay={0.4} className="w-full"><div className="flex items-center w-full gap-2 py-2"><hr className="w-full border-border"/><span className="text-xs font-semibold text-muted-foreground">OR</span><hr className="w-full border-border"/></div></BlurFade>
                    </motion.div>}
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground">Create password</p></BlurFade>
                        <BlurFade delay={0.1}><p className="text-sm font-medium text-muted-foreground">Must be at least 6 characters.</p></BlurFade>
                    </motion.div>}
                     {authStep === "confirmPassword" && <motion.div key="confirm-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} className="w-full flex flex-col items-center text-center gap-4">
                         <BlurFade delay={0} className="w-full text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground">One Last Step</p></BlurFade>
                         <BlurFade delay={0.1}><p className="text-sm font-medium text-muted-foreground">Confirm your password</p></BlurFade>
                    </motion.div>}
                </AnimatePresence>
                ...
                <form onSubmit={handleFinalSubmit} className="w-full space-y-6">
                        ...
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3 }} className="w-full space-y-6">
                            <BlurFade delay={authStep === 'email' ? 0.5 : 0} className="w-full">
                                <div className="glass-input">
                                    <div className="w-10 pl-2 flex items-center justify-center"><Mail className="h-5 w-5 text-foreground/80" /></div>
                                    <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={handleKeyDown} className="h-12 flex-grow bg-transparent text-foreground placeholder:text-foreground/40 outline-none" />
                                ...
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="glass-input">
                                        <div className="w-10 pl-2 flex items-center justify-center">
                                            {isPasswordValid ? <button type="button" onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 text-foreground/80" />}
                                        </div>
                                        <input ref={passwordInputRef} type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="h-12 flex-grow bg-transparent text-foreground placeholder:text-foreground/40 outline-none" />
                                        <div className={cn("overflow-hidden transition-all duration-300", isPasswordValid ? "w-10 pr-1" : "w-0")}>
                                            <GlassButton type="button" onClick={handleProgressStep} size="icon"><ArrowRight className="w-5 h-5" /></GlassButton>
                                        </div>
                                    </div>
                                    <button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-xs text-foreground/50 hover:text-foreground transition-colors mx-auto"><ArrowLeft className="w-3 h-3" /> Go back</button>
                                </BlurFade>}
                            </AnimatePresence>
                        </motion.div>}
                    </AnimatePresence>
                    <AnimatePresence>
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" className="w-full">
                            <div className="glass-input">
                                <div className="w-10 pl-2 flex items-center justify-center">
                                    {isConfirmPasswordValid ? <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)}>{showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 text-foreground/80" />}
                                </div>
                                <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} placeholder="Confirm" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="h-12 flex-grow bg-transparent text-foreground placeholder:text-foreground/40 outline-none" />
